...
type Atividade = { atividade: string; empresa: string; status: string; obs: string; origem?: string; };
...
export default function RdoDetalhePage() {
  ...
  async function loadRdo() {
    ...
    setStatus(data.status || (data.draft ? "rascunho" : "finalizado"));
  ...
  async function handleSalvar(finalizar = false) {
    ...
    try {
      ...
      // Um RDO programado passa a "rascunho" assim que o técnico começa a
      // preencher — sem tela extra, é a primeira gravação que já promove.
      const novoStatus = finalizar ? "finalizado" : "rascunho";
      ...
      const { error } = await supabase.from("rdo_reports").update(rdoData).eq("id", rdoId);
      if (error) throw error;

      setStatus(novoStatus);
      showToast(finalizar ? "RDO Finalizado com sucesso!" : "Rascunho salvo com sucesso!");

      if (finalizar) router.push(`/mobile/projetos/${projectId}/rdo`);
      else await loadRdo();
  ...
  if (notFound) return <MobileShell title="Não encontrado" backHref={`/mobile/projetos/${projectId}/rdo`}><div className="p-10 text-center text-gray-500">RDO não encontrado.</div></MobileShell>;
  ...
  return (
    <MobileShell
      ...
      backHref={`/mobile/projetos/${projectId}/rdo`}
      actions={<RdoStatusBadge status={status} />}
    >
      <div className="space-y-6 pb-20 bg-gray-50 -m-4 p-4 min-h-screen">
        ...
        {status === "programado" && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-700 font-medium">
            Este RDO foi programado com antecedência. As atividades planejadas já estão listadas abaixo — assim que você salvar, ele passa para "Em preenchimento".
          </div>
        )}
            ...
            {atividades.map((a, i) => {
              ...
              return (
                <div key={i} className={`relative bg-white border border-gray-100 border-l-4 ${statusColor} p-4 rounded-xl shadow-sm space-y-3`}>
                  ...
                  {a.origem === "programada" && (
                    <span className="inline-block text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Programada</span>
                  )}
