...
export default function FisicoQuimicosDetalhePage() {
  ...
  // ================= GERAÇÃO DO PDF GERAL DA CAMPANHA (NOVO) =================
  async function baixarPdfGeral() {
    setIsGeneratingGeral(true);

    try {
      // 1. Busca todos os poços do projeto
      const { data: todosPocos } = await supabase
        .from("water_samplings")
        .select("*")
      ...
      if (!todosPocos || todosPocos.length === 0) {
        alert("Nenhuma amostragem encontrada neste projeto.");
        return;
      }

      // 2. Filtra os poços que têm a MESMA DATA da ficha que está aberta agora
      const dataDaFichaAberta = amostra.data;
      const pocosDoDia = todosPocos.filter(p => p.data === dataDaFichaAberta);

      if (pocosDoDia.length === 0) {
         alert("Nenhum poço encontrado para esta data.");
         return;
      }
      ...
      const drawPageHeader = () => {
        ...
        doc.text(`DATA DA CAMPANHA: ${formatDateBr(dataDaFichaAberta)}`, pageWidth - marginX, 28, { align: "right" });
      ...
      // Desenha as tabelas de cada poço do dia
      for (let i = 0; i < pocosDoDia.length; i++) {
        const amostraDia = pocosDoDia[i];
        ...
        doc.text(`POÇO: ${amostraDia.poco} | ${amostraDia.nomenclatura || "Sem nomenclatura"}`, marginX, currentY);
        ...
        const infoLinha1 = `Cód: ${amostraDia.identificacao_codigo || "-"}   |   Hora Início: ${amostraDia.hora_inicio || "-"}   |   NA Inicial: ${amostraDia.na_inicial || "-"} m   |   NA Final: ${amostraDia.na_final || "-"} m`;
        doc.text(infoLinha1, marginX, currentY);
        currentY += 5;

        if (amostraDia.fase_livre) {
          doc.setTextColor(200, 0, 0);
          doc.setFont("helvetica", "bold");
          doc.text(`Fase Livre: DETECTADA (Espessura: ${amostraDia.espessura_fl || "-"} m)`, marginX, currentY);
        } else {
        ...
        autoTable(doc, {
          ...
          body: amostraDia.leituras?.map((l: any) => [
            l.horario || "-", 
            l.na || "-", 
            l.ph || "-", 
            l.orp || "-", 
      ...
      doc.save(`Compilado_FQ_${projectName}_${dataDaFichaAberta}.pdf`);
  ...
  // ================= GERAÇÃO DO PDF INDIVIDUAL (MANTIDA IGUAL) =================
          ...
          <div className="bg-[#391e2a] px-8 py-6 text-white flex flex-col md:flex-row justify-between md:items-center gap-4 shadow-inner">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button onClick={gerarPDF} className="bg-[#80b02d] hover:bg-[#6c9526] text-white font-bold shadow-lg">
                <Icons.Download /> Gerar PDF deste Poço
              </Button>
              {/* NOVO BOTÃO: BAIXAR PDF GERAL */}
              <Button onClick={baixarPdfGeral} disabled={isGeneratingGeral} className="bg-gray-100 hover:bg-gray-200 text-[#391e2a] font-bold shadow-lg">
                <Icons.Download /> {isGeneratingGeral ? "Gerando..." : "Baixar PDF Geral do Dia"}
              </Button>
            </div>
          </div>
