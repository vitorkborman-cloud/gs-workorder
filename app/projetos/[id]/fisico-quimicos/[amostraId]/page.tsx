...
export default function FisicoQuimicosDesktopPage() {
  ...
  // ================= MÁGICA: GERAR PDF GERAL DA PÁGINA INICIAL =================
  ...
  // ================= PDF INDIVIDUAL DA TABELA =================
  async function baixarPdfIndividual(amostra: Sampling) {
    setGeneratingPdf(`indiv_${amostra.id}`); 
            ...
            {datasAgrupadas.map((dataCampanha) => {
              ...
              return (
                <div key={dataCampanha} className="bg-white rounded-3xl shadow-md border border-gray-200 overflow-hidden transition-all duration-300">
                  ...
                  {/* CABEÇALHO DO CARD (DATA) E O BOTÃO "BAIXAR PDF GERAL" (Como no Print) */}
                    ...
                    <table className="w-full text-sm text-left">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {amostrasDoDia.map((amostra) => {
                          const isGeneratingIndiv = generatingPdf === `indiv_${amostra.id}`;
                          ...
                          return (
                            <tr key={amostra.id} className="hover:bg-gray-50/50 transition-colors group">
                              ...
                              <td className="px-8 py-4 text-right">
                                ...
                                {/* BOTÕES DE AÇÃO: Baixar Individual e Ver Ficha */}
                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  
                                  <button 
                                    onClick={() => baixarPdfIndividual(amostra)}
